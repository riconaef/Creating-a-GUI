...
class my_program:
    def __init__(self, master):
        ...
        self.tableFrame = Frame(self.left_frame, width=50, height=50, bg='grey')
        self.tableFrame.grid(row=17, column=0, columnspan=6, rowspan=20)
        ...
        self.plot_tableraw_button = Button(self.left_frame, text="raw data", command=self.plot_tableraw)
        self.plot_tablecalc_button = Button(self.left_frame, text="calculated data", command=self.plot_tablecalc)
        ...
        self.plot_tableraw_button.grid(row=3, column=5, padx=5, pady=5)
        self.plot_tablecalc_button.grid(row=4, column=5, padx=5, pady=5)
        ...
        # create three lists for the calculation of the modules.
        # These lists contain the type of module, start and end point.
        ...
        self.colnames = ['Pres [kPa]', 'arm1 [mm]', 'arm2 [mm]', 'arm3 [mm]', 'arm4 [mm]', 'arm5 [mm]', 'arm6 [mm]',
                         'AVG [mm]']
        self.tree['columns']=self.colnames
        self.tree.column('Pres [kPa]', anchor='center', width=100)
    ...
    def import_data(self):
        ...
        self.arm1 = pd.to_numeric(self.df['Arm 1'], downcast="float")
        self.arm2 = pd.to_numeric(self.df['Arm 2'], downcast="float")
        self.arm3 = pd.to_numeric(self.df['Arm 3'], downcast="float")
        self.arm4 = pd.to_numeric(self.df['Arm 4'], downcast="float")
        self.arm5 = pd.to_numeric(self.df['Arm 5'], downcast="float")
        self.arm6 = pd.to_numeric(self.df['Arm 6'], downcast="float")
        self.arm_avg = (self.arm1 + self.arm2 + self.arm3 + self.arm4 + self.arm5 + self.arm6) / 6
        self.df_data = pd.concat([self.pres, self.arm1, self.arm2, self.arm3, self.arm4, self.arm5, self.arm6,
                                  self.arm_avg], axis=1)
    ...
    def calculate_modules(self):
        ...
        for x in range(len(self.mode)):
            if self.mode[x].get() != '':
                self.test_type[self.mode[x].get()] = np.array([int(self.start[x].get()), int(self.end[x].get())])
        ...
        for case in self.test_type:
            ...
            sd = (self.arm_avg[self.test_type[case][0]] * 2 + d_sond) / ((self.arm_avg[self.test_type[case][1]] -
                                                                          self.arm_avg[self.test_type[case][0]]) * 2)
            ...
            table_temp = pd.concat([table_temp], keys=[str(case)])
            self.df_calc = pd.concat([self.df_calc, table_temp])
            ...
            table_temp_raw = pd.DataFrame({'': counter, 'Pressure [kPa]': pres_raw, 'Arm 1 [mm]': arm1_raw,
                                           'Arm 2 [mm]': arm2_raw, 'Arm 3 [mm]': arm3_raw, 'Arm 4 [mm]': arm4_raw,
                                           'Arm 5 [mm]': arm5_raw, 'Arm 6 [mm]': arm6_raw})
                
            self.df_raw = pd.concat([self.df_raw, table_temp_raw])
    ...
    def plot_average(self):
        ...
        points_avg = self.ax1.scatter(self.arm_avg, self.pres, label='Average arms', facecolors='royalblue',
                                      edgecolors='black')
        ...
        cursor.connect("add", lambda sel: sel.annotation.set_text('Point ' + str(labels[sel.index])))
        ...
        try:
            ...
            for i in self.test_type:
                ...
                self.ax1.annotate(list(self.test_type.keys())[count1], (self.arm_avg[self.test_type[i][0]],
                        self.pres[self.test_type[i][0]]), fontsize=15, textcoords="offset points", xytext=(10, -10))
                count1 += 1
    
            count2 = 0
            text_final = ''
            for ii in self.test_type:
                text_final += (ii + ': G = ' + str(self.g[count2]) + ' MPa, E = ' + str(self.e[count2]) +
                               ' MPa' + str("\n") + str("\n"))
                count2 += 1
    ...
    def plot_allarms(self):
        ...
        self.ax1.legend(fontsize=15, loc='lower right');
        mplcursors.cursor([self.points_all1, self.points_all2, self.points_all3, self.points_all4, self.points_all5,
                           self.points_all6])
    ...
    def plot_armpairs(self):
        ...
        self.ax1.cla()
        self.points_pair1_4 = self.ax1.scatter((self.arm1+self.arm4)/2, self.pres, facecolors='yellow',
                                               edgecolors='black', label='Armpair 1 + 4')
        self.points_pair2_5 = self.ax1.scatter((self.arm2+self.arm5)/2, self.pres, facecolors='red',
                                               edgecolors='black', label='Armpair 2 + 5')
        self.points_pair3_6 = self.ax1.scatter((self.arm3+self.arm6)/2, self.pres, facecolors='blue',
                                               edgecolors='black', label='Armpair 3 + 6')
        self.ax1.legend(fontsize=15, loc = 'lower right');
    ...
    def plot_tableraw(self):
        # Display the raw data from which the modules are calculated
        self.newWindow = Toplevel()
        self.tree = ttk.Treeview(self.newWindow, show='headings')
        self.tree.pack(side='bottom', fill='both', padx=10, pady=5, expand=True)

        #self.df_raw.round(3)
        self.table = pt = Table(self.tree, dataframe=self.df_raw, showtoolbar=True, showstatusbar=True)
        
        pt.show()
        
        
    def plot_tablecalc(self):
        # Display the calculated modules in a table
        self.newWindow1 = Toplevel()
        self.tree1 = ttk.Treeview(self.newWindow1, show='headings')
        self.tree1.pack(side='bottom', fill='both', padx=10, pady=5, expand=True)

        self.table1 = pt = Table(self.tree1, dataframe=self.df_calc.round(3),
